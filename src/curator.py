...
class CuratorState:
    ...
    # Replaces cumulative pnl_since_switch which could mask later losses.
...
class CuratorAgent:
    def __init__(
        ...
        # Product B: Consensus gate settings (loaded from profile + defaults)
        self.consensus_defaults: dict = cfg.get("consensus_defaults", {})
        self._profile_consensus_cache: dict[str, dict] = {}
    ...
    def consensus_settings(self) -> dict:
        ...
        Merges profile-specific settings with global defaults.
        """
        profile_name = self.state.current_profile
        if profile_name in self._profile_consensus_cache:
            return self._profile_consensus_cache[profile_name]
        
        profile = self.profiles.get(profile_name, {})
        defaults = self.consensus_defaults
