...
class OrderExecutor:
    ...
    async def place_order(
        ...
        # Live position context: the reduce-only check (gate check 8) needs to
        # know which side we are currently on. Without it the check is inert —
        # a sell could flip a long to a short and the gate would never know.
        # Best-effort: a failed/unavailable position read degrades to None
        # (check inert for this order, same as before), never blocks on a
        # transport failure. Skipped in dry-run (no live position exists) and
        # for orders that already declare reduce_only (they pass by design).
        current_position_side = None
        if not self.dry_run and not order.reduce_only:
            current_position_side = await self._live_position_side(order.inst_id)
    ...
    async def _live_position_side(self, inst_id: str) -> str | None:
        """Resolve the current position side ("long" | "short" | None) for an
        instrument, used by the gate's reduce-only check.

        Handles both OKX long/short (hedge) mode via `posSide` and net mode
        via the sign of `pos`. Any read failure or zero/absent position
        returns None — the caller treats that as no position context.
        """
        pos = await self.get_position(inst_id)
        if not pos:
            return None
        try:
        ...
        if amt == 0:
            return None
        side = (pos.get("posSide") or "").strip().lower()
