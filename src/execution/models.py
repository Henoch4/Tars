...
class OrderRequest:
    ...
    # None = caller declares no leverage information (e.g. SPOT); the gate
    # cannot second-guess what it was never told, same policy as
    # confidence_bps. SWAP callers SHOULD set this from the instrument's
    # configured leverage (OKX `account/set-leverage` / get-leverage).
    leverage: float | None = None
    # Intended price from signal generation (mid/last price). Used for
    # pre-trade fat-finger check on market orders: if the agent's view of
