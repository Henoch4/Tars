...
## Project: TARS — Trade Audit & Risk System
...
TARS is the robot from *Interstellar*. The name fits this project perfectly — **Trade Audit & Risk System**: a trading robot with rules it cannot break, and a public record of every decision. Before every trade, it writes down what it's about to do in a public record. If the write fails, the trade doesn't happen.
...
- **Free on testnet**: No fees today — payment wiring built but deliberately switched off
- **Self-custodial**: Uses OKX OAuth/demo mode — no private key handling

## Backronym

**TARS = Trade Audit & Risk System**. The robot cannot exceed strict limits set in advance. Those numbers live in a separate part of the system the trading code can't reach, and they can only ever be made stricter, never looser.
