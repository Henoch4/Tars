...
def _agent(**overrides):
    # D5 (a): spot companions must be explicitly declared; the arb package's
    # spot leg BTC-USDT is not implicitly allowed by BTC-USDT-SWAP.
    default_gate = RiskGate(
        max_position_usd=5000,
        allowed_assets=["BTC-USDT-SWAP", "ETH-USDT-SWAP", "SOL-USDT-SWAP", "BNB-USDT-SWAP"],
        allowed_companions=["BTC-USDT", "ETH-USDT", "SOL-USDT", "BNB-USDT"],
    )
    kwargs = dict(
        okx_cli=_StubCli(),
        risk_gate=default_gate,
        dry_run=True,
        max_position_usd=5000,
