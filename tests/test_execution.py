...
class TestRiskGate:
    ...
    def test_spot_leg_rejected_without_explicit_companion(self):
        """D5 (a) exact match + explicit companions: the funding-arb spot leg
        `BTC-USDT` is NOT implicitly authorized by `BTC-USDT-SWAP`. Without an
        explicit companion entry the spot leg must be rejected."""
        gate = RiskGate(allowed_assets=["BTC-USDT-SWAP"])
        order = OrderRequest(
            inst_id="BTC-USDT",  # spot form, same base asset — not implicitly allowed
            side="buy",
            order_type="market",
            size="1000",
        )
    ...
    def test_spot_companion_explicitly_allowed(self):
        """D5 (a): with an explicit companion, the spot leg is allowed."""
        gate = RiskGate(
